...
function RateLimit(options) {
    ...
    options = defaults(options, {
        ...
        delayMs: 1000, // milliseconds - base delay applied to the response - multiplied by numbe r of recent hits for the same key.
    ...
    options.store = new RedisStore(options.store);
    ...
    function rateLimit(req, res, next) {
        ...
        options.store.incr(key, function(err, current, ttl) {
          if (err) {
              return next(err);
            }
            ...
            if (options.headers) {
              ...
              res.setHeader('X-RateLimit-Reset', Math.ceil((ttl + Date.now()) / 1000));
            }
